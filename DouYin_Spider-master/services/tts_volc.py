...
# ---------------------------------------------------------------------------
_HINTS = {
    45000010: '凭据无效 —— 方舟的 ark- 开头的 key 不能用（那条线不做 TTS）；'
              '要用「语音技术 / 豆包语音」控制台的 API Key（console.volcengine.com/speech）。',
    # ⚠️ 2026-09-17 实测更正：这个码**不是**「漏传 X-Api-Resource-Id」。
    #    带对了 header 一样报它（且方括号里会把别名归一化，说明 header 被正确解析了），
    #    真实含义是「请求的那个商品没被授予」= 账号未开通 / 该 API Key 不含该服务范围。
    45000030: '★ 资源未授权：账号没有开通对应的语音合成商品，或这把 API Key 没包含该服务。'
              '去 console.volcengine.com/speech 的「开通管理」开通'
              '「豆包语音合成大模型 2.0（字符版）」（首次可领试用额度）；'
              '已开通就检查 API Key 管理里这把 key 勾选的服务范围。',
    45000000: '鉴权 / 音色未授权 —— 音色没在控制台的「音色管理」里开通，'
              '或 Resource ID 与音色版本不匹配（2.0 音色必须配 seed-tts-2.0）。',
